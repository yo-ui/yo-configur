...
class Library {
	...
	init(stage) {
      ...
      {name:'通用',type:1,icon:'fa-base',
        subList:[
          ...
          {className:'Switch',title:'开关控件',url:'static/images/configur/switch.png',width:100,height:30},
          {className:'State',title:'多态控件',url:'static/images/configur/state.png',width:30,height:30},
      ...
      {name:'图表',type:1,icon:'fa-chart',
        subList:[
          ...
          {isHead: true,name: '其他',
            subList: [{className:'Kydt',title:'客用电梯',url:'static/images/configur/kydt.png',width:50,height:70}]},
          {isHead: true,name: '管道',
            subList: [{className:'LinkPoint',title:'连接点',url:'static/images/configur/link-point.png',width:10,height:10},
      ...
      {name:'素材',type:1,icon:'fa-decorate',
        subList:[
          ...
          {className:'Decorate1',title:'装饰1',url:'static/images/configur/d-1.png',width:300,height:40,max:true},
          {className:'Decorate2',title:'装饰2',url:'static/images/configur/d-2.png',width:300,height:40,max:true},
          ]},
		]
	...
	create(index) {
		...
		dataList.forEach(function(sub) {
		  if(sub.isHead) {
        let tree = $(`<div class="bm-tree"><i class="fa fa-down"></i>&nbsp;${sub.name}</div>`);
        $('.bm-component-list').append(tree);
        let content = $(`<div></div>`)
        if(sub.subList) {
          sub.subList.forEach(function (data) {
            let piece = that.createData(data)
            content.append(piece);
          })
        }
        $('.bm-component-list').append(content);
      }else {
        let piece = that.createData(sub)
        $('.bm-component-list').append(piece);
      }
		});
    $('.bm-component-list .bm-tree').each(function () {
      Tree.init($(this))
    })
	}
    ...
    let piece = $(`<div class="piece"></div>`).append(img).append(`<p>${that.textFormat(sub.title, 4)}</p>`);
    if(sub.max) {
      piece.addClass('max');
    }
    return piece;
