...
export default {
  ...
  methods: {
    ...
    show(item = {}) {
      let { condition } = this;
      let { type = "", rule = "", callback = () => {} } = item || {};
      condition.dataType = type;
      condition.ruleType = rule;
      condition.callback = callback;
      let ruleContent = "";
      let tip = "";
      if (type == "boolean") {
        ruleContent = "true,false";
        tip = "使用英文逗号分割字符串'true'或'false'";
      } else if (type == "date") {
        let date = this.$moment();
        let start = date.format("YYYY-MM-DD HH:mm:ss");
        let end = date.add(30, "m").format("YYYY-MM-DD HH:mm:ss");
        ruleContent = `${start},${end}`;
        tip = `使用英文逗号分割符合规范的日期字符串，如 ${start}, ${end}`;
      } else if (type == "number") {
        ruleContent = "1,3,5,7,9";
        tip = "使用英文逗号分割数字，如 1,3,5,7,9";
      } else if (type == "string") {
        ruleContent = "北京,上海,深圳";
        tip = "使用英文逗号分割一系列字符串，如 first,second,third";
      }
      condition.tip = tip;
      condition.ruleContent = ruleContent;
