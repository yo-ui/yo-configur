<template>
  <div class="bm-countdown-text-style-com">
    <!-- <h2>{{ $lang("功能选择") }}</h2>
    <p>
      <i
        class="el-icon-rank"
...
export default {
  name: "countdownTextStyleCom",
  data() {
    let formatList = Object.freeze([
      { code: "{dd}天{hh}小时{mm}分{ss}秒", name: "天小时分秒" },
      { code: "{hh}小时{mm}分{ss}秒", name: "小时分秒" },
      { code: "{mm}分{ss}秒", name: "分秒" },
      { code: "{ss}秒", name: "秒" }
    ]);
    return {
      activeNames: ["name"],
