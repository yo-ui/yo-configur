...
export default {
  ...
  getters: {
    ...
    // },
    //获取缓存数据对象
    getPointValueSelectCacheMap(state) {
      let pointValueSelectCacheMap = state.pointValueSelectCacheMap || {};
      return key => {
        return pointValueSelectCacheMap[key];
      };
    }
    // //获取缓存数据对象
  ...
  mutations: {
    ...
    // },
    //设置缓存对象
    setPointValueSelectCacheMap(state, { key, value } = {}) {
      let pointValueSelectCacheMap = JSON.parse(
        JSON.stringify(state.pointValueSelectCacheMap || {})
      );
      pointValueSelectCacheMap[key] = value;
      state.pointValueSelectCacheMap = pointValueSelectCacheMap;
    }
    // //设置缓存对象
