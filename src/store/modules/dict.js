...
export default {
  ...
  getters: {
    ...
    // },
    //获取缓存数据对象
    getDictListDataMap(state) {
      let dictListDataMap = state.dictListDataMap || {};
      return key => {
        return dictListDataMap[key] || {};
      };
    },
    // //获取缓存数据对象
  ...
  mutations: {
    ...
    // },
    //设置缓存对象
    setDictListDataMap(state, { key = "", value = {} } = {}) {
      let dictListDataMap = JSON.parse(
        JSON.stringify(state.dictListDataMap || {})
      );
      dictListDataMap[key] = value || {};
      state.dictListDataMap = dictListDataMap;
    },
    // //设置缓存对象
