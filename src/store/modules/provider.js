...
export default {
  ...
  getters: {
    ...
    // },
    //获取缓存数据对象
    getProviderListCacheMap(state) {
      let providerListCacheMap =
        state.providerListCacheMap || {};
      return key => {
        return providerListCacheMap[key];
      };
    },
    //获取缓存数据对象
  ...
  mutations: {
    ...
    // },
    //设置缓存对象
    setProviderListCacheMap(state, { key, value } = {}) {
      let providerListCacheMap = JSON.parse(
        JSON.stringify(state.providerListCacheMap || {})
      );
      providerListCacheMap[key] = value;
      state.providerListCacheMap = providerListCacheMap;
    },
    //设置缓存对象
