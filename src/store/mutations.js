...
export default {
  ...
  setAccountMenuMap(state, item) {
    if (item) {
      ...
      bmCommon.removeItem(Constants.LOCALSTORAGEKEY.ACCOUNTMENUMAP);
    }
    state.accountMenuMap = item;
  },
  //设置用户菜单列表
  setAccountMenuList(state, item) {
    if (item) {
      bmCommon.setItem(
      ...
      bmCommon.removeItem(Constants.LOCALSTORAGEKEY.ACCOUNTMENULIST);
    }
    state.accountMenuList = item;
  }
};
